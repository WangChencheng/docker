...
func checkPostgresVersion(db *sql.DB, server string, querySql string, versionRegex regexp.Regexp) (semver.Version, string, error) {
	...
	semanticVersion, err := parseVersion(versionString, versionRegex)
	if err != nil {
...
func (e *Exporter) checkMapVersions(ch chan<- prometheus.Metric, server *Server) error {
	semanticVersion, versionString, err := checkPostgresVersion(server.db, server.String(), "SELECT setting from pg_config where name='VERSION'", versionRegex)
	...
	hgSemanticVersion, hgVersionString, err := checkPostgresVersion(server.db, server.String(), "SELECT version()", regexp.MustCompile(`\w+ ((\d+)(\.\d+)(\.\d+)?)`))
	// Output the version as a special metric only for master database
	versionDesc := prometheus.NewDesc(fmt.Sprintf("%s_%s", namespace, staticLabelName),
		"Version string as reported by postgres", []string{"version", "short_version", "pg_version", "pg_short_version"}, server.labels)

	if !e.disableDefaultMetrics && server.master {
		ch <- prometheus.MustNewConstMetric(versionDesc,
			prometheus.UntypedValue, 1, hgVersionString, hgSemanticVersion.String(), versionString, semanticVersion.String())
	}
	return nil
}
